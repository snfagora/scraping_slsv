...
sc_binded <- sc_binded %>%
  mutate(numeric_score = case_when(
    score == "high" & index > 0 ~ 1, 
    score == "low" & index > 0 ~ 0.5,
    .default = 0
  )) 

sc_binded %>%
  group_by(type, numeric_score) %>%
  count()
```
...
sc_wide <- sc_binded %>%
  pivot_wider(names_from = type, 
              values_from = numeric_score, 
              values_fill = 0)

sc_wide %>%
  select(accountability, learning, strategic_capacity) %>%
  cor() %>%
...
```{r}
write_csv(matched_copy, here("processed_data", "matched.csv"))
```

```{r}
text4class <- ap %>% left_join(matched %>% select(-c("name.x")), by = c("name" = "name.y",
                                 "text" = "text"))

text4class <- text4class %>%
  mutate(relevant = ifelse(numeric_score > 0, 1, 0)) %>%
  select(name, text, type, relevant) %>%
  distinct()
```

```{r}
write_csv(text4class, here("processed_data", "text4class.csv"))
```